 ...
 *  1. pom.xml의 dependency에 commons-io, commons-fileupload가 명시되어있어야 한다. 
 *  2. servlet-context.xml에 MultipartResolver bean 설정이 있어야한다.
 *  3. MultipartFile 객체를 컨트롤러에서 받아서 사용한다.
 */
@Controller
public class FileController {
	...
	public String upload2(FileDto dto, HttpServletRequest request) {
		//FileDto로부터 업로드된 파일의 정보를 담고있는 MultipartFile 객체의 참조값 얻어내기
		...
		try {
			//3. 임시폴더에 저장된 업로드 파일을 원하는곳에 원하는 이름으로 이동시키기(전송)
