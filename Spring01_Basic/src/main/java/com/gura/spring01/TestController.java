...
public class TestController {
	...
	public String test() {
		/*
		 * Controller 메소드에서 return type을 String으로 설정 후
		 * 문자열을 리턴해주면 해당 문자열은 view page (jsp) 페이지의 위치가 된다.
		 * 즉 해당 jsp 페이지로 자동 forward 이동 되어서 응답한다.
		 */
	...
	public String update() {
		...
		// "redirect: 이동할 절대경로(context path는 쓰지 않는다)
		return "redirect:/move/test";
	}
	
	@RequestMapping("/move/fortune")
	public ModelAndView fortune() {
		//Model 과 view page의 위치를 동시에 담을 수 있는 ModelAndView 객체 생성
		ModelAndView mView = new ModelAndView();
		
		//view page에 전달할 모뎅(데이터)라고 가정
		String fortuneToday = "동쪽으로 가면 귀인을 만나요.";
		
		// HttpServleRequest 객체에 담는 대신 .addObject(key, value) 형태로 ModelAndView 객체에 담으면 된다.
	...
	// ModelAndView 객체가 필요하다면 직접 생성하지 않고 메소드의 매개변수로 선언하면 자동을 전달된다.
