...
public class ShopServiceImpl implements ShopService{
	...
	public void getList(ModelAndView mView) {
		...
		//ModelAndView 객체에 list라는 키값으로 담기
	 ...
	 *  3. 트랜젝션을 관리할 서비스의 메소드에 @Transactional 어노테이션 추가
	 *  
	 *  - 프로그래머의 의도하에 트랜젝션에 영향을 주는 Exception을 발생시키는 방법
	 *  
	 *  DataAccessException 클래스를 상속받은 클래스를 정의하고
	 *  ex)class MyException extends DataAccessExeption{ }
	 *  
	 *     throw new MyException("예외 메세지");
	 *     
	 *  예외를 발생시킬 조건이라면 위와 같이 예외를 발생시켜 트랜젝션이 관리되도록 한다.
	 */
	@Transactional
	@Override
